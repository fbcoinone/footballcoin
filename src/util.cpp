...
const char * const BITCOIN_CONF_FILENAME = "footballcoin.conf";
const char * const BITCOIN_PID_FILENAME = "footballcoin.pid";
...
#else
    const char* pszModule = "footballcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FootBallCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FootBallCoin";
#else
    // Unix
    return pathRet / ".footballcoin";
#endif
#endif
}
